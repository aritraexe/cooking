  ...
  const content = (
    <>
      <div className="flex items-start justify-between gap-3">
        ...
        {isAvailable ? <span className="flex shrink-0 items-center gap-1 rounded-full border border-accent/25 bg-accent/5 px-2 py-1 text-[11px] text-accent"><Check className="h-3 w-3" aria-hidden="true" /> {modeLabel}</span> : <span className="flex shrink-0 items-center gap-1 rounded-full border border-line px-2 py-1 text-[11px] text-ink-muted"><Clock3 className="h-3 w-3" aria-hidden="true" /> Coming soon</span>}
      </div>
      <div className="mt-4 flex items-center justify-between border-t border-line/70 pt-3 text-xs">
        <span className={isAvailable ? 'text-accent' : 'text-ink-muted'}>{isAvailable ? 'Open local tool' : operation.processingMode === 'LOCAL_AI' ? 'Coming soon · local AI' : operation.processingMode === 'LOCAL_HEAVY' ? 'Coming soon · local engine' : 'Coming soon · local processor'}</span>
...
export function AllTools() {
  ...
  const [scope, setScope] = useState<'all' | 'available' | 'quick' | 'popular'>('all')
